...
![Horse populations for all provinces in Canada from 1906 - 1972.](../results/horse_pops_plot.png){width=65% #fig-horse_pop_provs}


We can see from @fig-horse_pop_provs that Ontario, Saskatchewan and Alberta 
...
We present the standard deviations in @tbl-horse_stdev.

```{python}
#| label: tbl-horse_stdev
...
Additionally, note that in @tbl-horse_stdev we consider the sample standard deviation of the number of horses during the same time span as Figure 1.

![Horse populations for the province with the largest standard deviation](../results/horse_pops_plot_largest_sd.png){width=65% #fig-horse_largest_stdev}

In @fig-horse_largest_stdev we zoom in and look at the province of `{python} largest_sd`, which had the largest spread of values in terms of standard deviation.
